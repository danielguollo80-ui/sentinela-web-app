  ...
  try {
    ...
    if (botType === 'stocks' && symbol) {
      ...
      if (fullData?.[sym]) {
        ...
        // Injeta 1H/15M/5M (e deriva 4H) se o bot não calculou
        if (!cached.indicators_1h) {
          try {
            ...
            const [r1h, r15m, r5m] = await Promise.all([
