...
export async function GET(request: Request) {
  try {
    ...
    if (botType === 'stocks' && symbol) {
      ...
      // Return cached data + inject pre-market live + multi-TF
      if (fullData?.[sym]) {
      ...
      try {
        ...
        // Multi-TF: busca 1W/1H/15M/5M do Yahoo Finance e deriva 4H agregando 1H
        {
          const [res1w, res1h, res15m, res5m] = await Promise.all([
            fetch(`https://query2.finance.yahoo.com/v8/finance/chart/${sym}?interval=1wk&range=5y`, { headers, next: { revalidate: 86400 } }),
          ...
          if (res1w.ok) { const d = extractCandles(await res1w.json()); if (d) realtimeData.indicators_1w = calcTFStock(d.c, d.h, d.l, d.v); }
          if (res1h.ok) {
